...
def per_class_iu(hist, unique_label):
...
class IoU(Metric):
    def __init__(self, dataset_config, dist_sync_on_step=False, compute_on_step=True):
        ...
        self.unique_label = np.asarray(sorted(list(self.SemKITTI_label_name.keys())))[0:]
    ...
    def compute(self):
        iou = per_class_iu(sum(self.hist_list), self.unique_label)
